...
int main() {
   ...
   */
  pid_steering.Init(0.07, 0.003, 1.5);
  pid_throttle.Init(2.5, 0.0, 0.0);
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // std::cout<<"Speed error: "<<pid_throttle.TotalError()<<std::endl;
          throttle_value = fmax(-0.5, 1.0 + pid_throttle.TotalError()); 
          // std::cout<<"Throttle: "<<throttle_value<<std::endl;
          throttle_value = fmin(0.9, throttle_value);
          // DEBUG
